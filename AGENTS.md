...
- `src/derived/topic-normalization-patterns.tsv`: manually curated normalization policy for topic construction, exact migrations, display rules, aliases, and exceptions.
...
Use `src/derived/topic-normalization-patterns.tsv` as the detailed source of truth for normalization-owned topic construction, exact migrations, display rules, aliases, and exceptions. The curator and auditor read it but do not edit it: they resolve new slugs through active `creation` rules, apply active exact `migration` mappings only inside the owned shard, preserve established slugs unless an active shared rule deprecates them, and leave `review`, disabled, or ambiguous candidates unchanged for the handoff. They must not edit `topics.json`, another shard, redirects, generated data, or shared validation output, and must never invoke the corpus-wide `normalize:video-topics:apply` command.
