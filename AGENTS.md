...
at least two independent, sequential single-agent `$naval-site-content-auditor` tasks for each resulting exact shard. Each curator and auditor task writes its selected shard, passes the scoped `check:site-content-wording -- --path <shard> --strict --review` check, and runs `C:\Program Files\nodejs\npm.cmd run sync:video-topics` with sandbox elevation on the first attempt before recording successful completion. Curators append their processing-log row only after synchronization succeeds. Auditors always append exactly one truthful outcome row before handing back, including when an issue prevents completing those steps. Finish each task's finalization before the next task begins. Keep `fetch:video-metadata` as the independently rerunnable official-metadata repair command. Inventory/metadata and caption
...
Schedules, reports, generated archives, and logs other than `src/derived/site-content-processing.log` are coordinator-owned shared outputs, not shard-worker outputs. The scoped read-only `npm run check:site-content-wording -- --path <shard> --strict --review` command is the curator validation exception. Auditors also run the exact-shard timestamp sorter and canonical validation required by their skill. The deterministic `npm run sync:video-topics` command is the sole additional shared-source writer; never inspect or hand-edit `topics.json`. The transcript consumer passes the scoped wording check, runs `C:\Program Files\nodejs\npm.cmd run sync:video-topics` with sandbox elevation on the first attempt, then appends exactly one result line after successful synchronization. Auditors follow the skill's ordered wording, sorting, canonical-validation, and synchronization gates for successful completion, including unchanged, saturated, and intentionally empty results, and always append exactly one truthful outcome row even when an issue stops those steps. The selected shard does not require a pre-write drift check. Resolve every high-confidence wording issue and every review or ambiguous topic problem before recording successful completion; wording review candidates require transcript-grounded adjudication but do not need to reach zero. For curators, a wording-check failure, elevation denial, or synchronization failure blocks the completion row without rolling back or invalidating a completed shard write. For auditors, any unresolved issue blocks successful completion and must be recorded in the mandatory outcome row, with the actual changes, coverage, failed stage and reason, and required steps not run. Failures never waive audit logging or require another user request. Normal curator and auditor work never rewrites earlier log rows. Repository-level video removal is the exception: when a video is removed from the episode inventory, transcript manifest, and shard set, remove every processing-log row for its former canonical shard path in the same cleanup. A lane automation may also write only its explicitly named lane-private log and schedule state. Use `src/derived/site-content-processing.config.json` read-only for first-pass content defaults, video-type handling, follow-up stages, and topic grouping.
...
Topic descriptions are optional manual metadata stored in the registry's `summary` field. An explicitly requested topic-description task may fill this field for selected, source-checked topics. Use compact reference wording that starts with the role or identifying characteristic, omits repeated topic names and opening articles, and retains useful distinctions or operators. Deterministic topic creation must initialize new registry records with a blank description and must never ask an AI process to generate, infer, refresh, normalize, or clear them. Synchronization must preserve existing nonblank summaries. Do not confuse a video's concise summary subset of topic slugs with topic-description prose.

Retain valid unused registry topics and their curated summaries and aliases. Zero authored usage is informational or, at most, a warning; it must not be an error or a requirement to delete topics. Remove records only through reviewed semantic consolidation or an explicit deletion request, preserving unique curated metadata when consolidating.

Use `src/derived/topic-normalization-patterns.tsv` as the detailed source of truth for steady-state topic creation, display, alias, and exception policy. The curator and auditor read it but do not edit it: they resolve new slugs through active `creation` rules, preserve established slugs unless the active creation policy canonicalizes them, and treat `review`, disabled, or ambiguous candidates affecting the selected shard as blockers to successful completion. Auditors still record these blockers in their mandatory outcome row. They must not manually edit `topics.json`, another shard, generated data, or shared validation output, and must not perform corpus-wide topic rewrites from a selected-shard workflow. The deterministic synchronizer is the only permitted registry writer.
...
For follow-up quality passes, use `.agents/site-content-auditor.md` and `$naval-site-content-auditor`. This audit pass requires an explicitly selected `<manifest.fileStem>.json` shard and must stop without edits when none is supplied. It writes any transcript-backed improvements directly to that shard. After the shard is complete, it runs the scoped `check:site-content-wording -- --path <shard> --strict --review` check, fixes every high-confidence issue, and adjudicates review candidates against the transcript. After the skill's exact-shard sorter and canonical validation succeed, it runs `C:\Program Files\nodejs\npm.cmd run sync:video-topics` with sandbox elevation on the first attempt. These gates also apply when the shard remains unchanged, is saturated, or is intentionally empty. Every audit attempt with an explicitly selected canonical shard path must append exactly one truthful, newline-terminated result row at the physical bottom of `src/derived/site-content-processing.log` before handing back, regardless of issues encountered. This includes failed reads, incomplete coverage, wording, sorting or validation failures, unresolved topics, denied elevation, and synchronization failures in the selected shard or another shard. Stop dependent operations when necessary and log the actual progress, blocker, and required steps not run. Use the existing four-field contract and exact local timestamp format, preserve all earlier rows, and count retries within one attempt as one outcome. Only claim successful completion after all required gates pass. If writing the log itself fails, attempt narrow authorized recovery and check for an already-written row before retrying an uncertain append. If the log remains unwritable, missing, or malformed, report that specific logging failure and the exact pending row without claiming it was written or recreating existing history. It must not run repository-wide generation, tests, Astro/Pagefind builds, or shared validation, and it must not manually edit any other shared or generated output. The repository owner performs those integration checks before push.
