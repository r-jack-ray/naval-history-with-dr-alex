...
at least two independent, sequential single-agent `$naval-site-content-auditor` tasks for each resulting exact shard. Each curator and auditor task writes its selected shard without acquiring the repository
writer lease. Curators and auditors run `C:\Program Files\nodejs\npm.cmd run sync:video-topics` with sandbox elevation on the first attempt, let that package command own its internal writer
coordination, and append the processing-log row only after synchronization succeeds. Neither process invokes `src/scripts/site-content-pipeline-lock.mjs`
directly or manages its token. Finish each task's finalization before the next task begins. Keep `fetch:video-metadata` as the independently rerunnable official-metadata repair command. Inventory/metadata and caption
