...
When constructing a new topic slug, reserve an exact terminal `<whole>-<fraction>-inch-gun` or `<whole>-<fraction>-inch-guns` shape for a decimal gun calibre, and use `to` for a gun-calibre range, for example `4-to-5-inch-guns`. Preserve established slugs even when their dash-separated form cannot express the visible punctuation. If a newly introduced non-decimal topic necessarily contains adjacent numeric tokens, keep the evidence-backed slug in the owned shard and call it out in the handoff for repository-owner title and alias review. Do not guess the punctuation, rename an established slug, inspect or edit `topics.json`, synchronize topics, or touch other shared output as part of a shard-only curation or audit run.
