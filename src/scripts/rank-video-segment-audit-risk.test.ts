...
test("CLI maps canonical processing states, isolates malformed shards, and emits renamed headers", async () => {
  ...
  try {
    ...
    await writeFile(path.join(segments, "school-functions_school1.json"), JSON.stringify(shard(records[5]!)), "utf8");
    await writeFile(logPath, [
      ...
      "2026-07-12T20:00:02;src/derived/video-segments/generic_generic1.json;audited;no;no transcript-visible Q and A",
      "2026-07-12T20:00:03;src/derived/video-segments/explicit_explicit1.json;audited;no;title still warrants review",
    ].join("\n"), "utf8");
    ...
    assert.match(generic?.join("\t") ?? "", /low_signal/u);
    ...
    assert.match(result.stderr, /shards=5 excluded_sasc_shards=1/u);
  } finally {
