...
Common options:
  ...
  --no-lease              For explicitly isolated log operations, do not create or inspect a lease.
  --build                  Compile TypeScript before the run command while holding the lease.

Examples:
  node src/scripts/site-content-pipeline-lock.mjs acquire --owner content-coordinator --recover-stale
