...
14. Use `src/derived/topic-normalization-patterns.tsv` as the detailed source of truth for topic construction and deprecation. Resolve every new slug through active rules whose scope includes `creation`. Replace active exact `migration` mappings already present in the selected shard's video-level and segment-level topic arrays, deduplicating only those arrays in first-seen order. Preserve evidence-backed subject specificity, including distinctions between generic calibres and named systems. Preserve established slugs unless an active shared rule explicitly deprecates them. Leave `review`, `disabled`, ambiguous, or inapplicable matches unchanged and identify unresolved review candidates in the handoff.
...
- The skill itself edits only that shard and appends its one required result line to `src/derived/site-content-processing.log`. It does not edit `src/derived/topic-normalization-patterns.tsv` or `topics.json`; acquire leases; inspect other active runs; claim schedules; generate reports or archives; synchronize shared topics; install dependencies; run tests, builds, audits, or validation; invoke the corpus-wide `normalize:video-topics:apply` command; or complete/reset queue state.
