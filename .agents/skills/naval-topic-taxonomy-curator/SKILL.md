...
- Prefer `npm run report:video-topic-usage` for routine candidate discovery; it already indexes title, slug, and alias forms and scores them with stemmed-token Jaccard plus character-bigram Dice. Use the installed npm `fast-fuzzy` only for a scoped supplemental spelling or transposition probe against those forms. Establish acronym expansions and referent identity from corpus or authoritative-source evidence.
- Optionally use Python 3.14 `SetSimilaritySearch` when the task specifically needs exhaustive exact Jaccard-threshold candidate discovery across normalized token sets. Record the tokenization and threshold, then inspect returned pairs through the repository report and semantic evidence.
