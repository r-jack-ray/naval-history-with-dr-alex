...
9. Write the final field values, then run the timestamp sorter for the selected shard, perform canonical shard validation, run the scoped wording check, synchronize topics, and finalize the processing log in the required order below. The sorter is the ordering mechanism; do not replace it with a manual ordering check.
