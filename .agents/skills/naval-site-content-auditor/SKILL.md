...
- After the final selected-shard write and before canonical shard validation, run the timestamp sorter for that exact shard from the repository root:
  ...
  This skill owns one exact shard, so invoke the sorter once with that shard path rather than its parent directory. Run the sorter with elevated privileges to avoid sandbox EPERM issues. The sorter may reorder the selected shard and writes `reports/video-segment-malformed-timestamps.tsv`. If it reports malformed timestamps or otherwise exits unsuccessfully, use the report to repair only the selected shard when the transcript supports a correction, then rerun the sorter. If the failure cannot be resolved within the selected-shard scope, stop finalization, do not synchronize topics or append the processing-log row, and report the blocker.
