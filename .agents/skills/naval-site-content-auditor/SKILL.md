...
- Do not run `generate:site-data`, `site:check`, `site:build`, Pagefind, `npm run check`, or any other repository-wide generation, test, build, audit, topic-rewrite, or validation command. The scoped read-only `npm run check:site-content-wording -- --path <shard> --strict --review` command and deterministic `npm run sync:video-topics` finalization are the sole validation and shared-command exceptions.
...
- Append exactly one newline-terminated, semicolon-separated data line after the current final line at the physical bottom of the existing `src/derived/site-content-processing.log` only after synchronization succeeds and the scoped wording check passes. Append this result line for every completed selected-file audit, whether the audit changed the shard, left it unchanged, found the existing content saturated, or confirmed an intentionally empty shard. A wording-check failure, synchronization failure, or elevation denial is a shared-output finalization blocker: do not append the completion row, and report the blocker without treating the completed shard write as invalid. Never prepend the entry or insert it beneath the header.
