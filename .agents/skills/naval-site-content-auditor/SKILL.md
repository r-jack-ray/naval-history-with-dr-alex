...
- Do not run `generate:site-data`, `site:check`, `site:build`, Pagefind, `src/scripts/validate-content-pipeline.ts`, `npm run check`, or any other repository-wide generation, test, build, audit, topic-rewrite, or validation command. The deterministic `npm run sync:video-topics` finalization is the sole permitted shared command.
