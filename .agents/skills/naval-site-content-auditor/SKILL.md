...
3. Use `src/derived/topic-normalization-patterns.tsv` as the detailed source of truth for topic construction and deprecation. Resolve every new slug through active rules whose scope includes `creation`. Apply active exact `migration` mappings only inside the selected shard's video-level and segment-level topic arrays, deduplicating only those arrays in first-seen order. Preserve evidence-backed subject specificity, including distinctions between generic calibres and named systems. Preserve established slugs unless an active shared rule explicitly deprecates them. Leave `review`, `disabled`, ambiguous, or inapplicable matches unchanged and identify unresolved review candidates in the handoff.
4. Keep video-level topics as a concise summary subset of the richer segment-level topics. Outside active catalog rules, investigate registry records, aliases, synonyms, or near-duplicates only when the repository owner's later synchronization or validation reports a taxonomy problem, or when the user explicitly requests taxonomy work.
...
- Do not run `generate:site-data`, `site:check`, `site:build`, Pagefind, `.codex/hooks/validate-content-pipeline.ps1`, `npm run check`, `normalize:video-topics:apply`, or any other repository-wide generation, test, build, audit, normalization-apply, or validation command.
- After completing the audit of the selected shard, append exactly one newline-terminated, semicolon-separated data line after the current final line at the physical bottom of the existing `src/derived/site-content-processing.log`. Append this result line whenever the selected file was processed, whether the audit changed the shard, left it unchanged, found the existing content saturated, or confirmed an intentionally empty shard. Omit the line only when no exact file was supplied or a blocker stopped the audit before the selected file could be processed. Never prepend the entry or insert it beneath the header.
...
Report the scope audited, the number or type of records strengthened, topic slugs added to the owned shard, active topic mappings applied, unresolved review or ambiguous candidates, whether the shard changed, the processing-log line appended, and any remaining transcript passages that need another focused pass. State that the normalization catalog, shared topic registry, corpus-wide normalization apply, synchronization, generation, tests, builds, and validation were intentionally not touched because the repository owner performs them before push. Mention topic-registry work only when a previously reported synchronization or taxonomy problem required intervention. If the selected file was fully processed without a shard edit, still append and report the result line. Do not append only when the audit stops before processing the selected file.
