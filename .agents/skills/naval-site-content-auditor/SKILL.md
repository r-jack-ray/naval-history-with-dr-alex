...
- Do not run `generate:site-data`, `site:check`, `site:build`, Pagefind, `npm run check`, or any other repository-wide generation, test, build, audit, topic-rewrite, or validation command. The deterministic `npm run sync:video-topics` finalization is the sole permitted shared command.
- Finish transcript comparison, apply every justified wording or content change directly to the selected shard, and validate the canonical shard write. Do not perform a pre-write drift check.
