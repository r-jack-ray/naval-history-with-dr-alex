...
- Treat every segment as a watch point into a Dr. Clarke video: preview what the reader will see or hear, identify the historical subject, and state the learning payoff. Explain a subject's naval connection when one matters, but do not require a naval connection.
...
5. Preserve every other shard and all generated outputs. The lease-protected deterministic topic synchronizer described below is the only permitted shared-source writer.
...
- Do not run `generate:site-data`, `site:check`, `site:build`, Pagefind, `.codex/hooks/validate-content-pipeline.ps1`, `npm run check`, or any other repository-wide generation, test, build, audit, topic-rewrite, or validation command. The deterministic `npm run sync:video-topics` finalization is the sole permitted shared command.
- Finish transcript comparison, apply every justified wording or content change directly to the selected shard, and validate the canonical shard write without acquiring, inspecting, waiting on, or holding the repository writer lease. The selected shard is independently owned and is not protected by `.tmp/site-content-pipeline.lock`; do not perform a pre-write drift check.
- After the selected shard write is complete, acquire `.tmp/site-content-pipeline.lock` as the shared-output lease with `.codex/hooks/site-content-pipeline-lock.mjs acquire --purpose site-content-audit --recover-stale` and record its token. Keep the lease only across topic synchronization and the processing-log append. Pass its token through `CONTENT_PIPELINE_LOCK_TOKEN` when running `npm run sync:video-topics` so the command joins the already-owned lease. Run synchronization even when the completed audit is unchanged, saturated, or confirms an intentionally empty shard; its corpus-wide result may be `added N topics` or `already current` and must not be attributed only to the selected shard.
- Append exactly one newline-terminated, semicolon-separated data line after the current final line at the physical bottom of the existing `src/derived/site-content-processing.log` only after synchronization succeeds. Append this result line for every completed selected-file audit, whether the audit changed the shard, left it unchanged, found the existing content saturated, or confirmed an intentionally empty shard. A lease or synchronization failure is a shared-output finalization blocker: do not append the completion row, release any acquired lease, and report it without treating the completed shard write as invalid. Never prepend the entry or insert it beneath the header.
- Release the lease in all success and failure paths; renew it before expiry if finalization is unusually long. Never bypass, delete, or recover an active unexpired lease.
...
- Use this low-freedom PowerShell pattern, supplying the five variables with the actual audit result:
...
Report the scope audited, the number or type of records strengthened, topic slugs added to the owned shard, active creation rules used, unresolved review or ambiguous candidates, whether the shard changed, the exact corpus-wide `npm run sync:video-topics` result, the processing-log line appended, and any remaining transcript passages that need another focused pass. State that the normalization catalog, manual registry editing, corpus-wide topic rewriting, generation, tests, builds, and validation were intentionally not touched. If the selected file was fully processed without a shard edit, still synchronize, append, and report the result line. Do not append when finalization is blocked before synchronization succeeds.
