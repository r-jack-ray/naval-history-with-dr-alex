...
- When the user explicitly requests corpus-wide taxonomy maintenance rather than one narrow build repair, route the work to `$naval-topic-taxonomy-curator`. That skill owns the report-driven semantic review and coherent policy, shard, and registry edits; routine site builds intentionally omit that backlog.
