...
9. After transcript comparison and draft preparation, apply every justified change directly to the selected shard and validate its canonical shard write without acquiring, inspecting, waiting on, or holding the repository writer lease. Then acquire the shared-output lease for only `npm run sync:video-topics` and the processing-log append; pass the token through `CONTENT_PIPELINE_LOCK_TOKEN` so synchronization joins the existing lease. Run synchronization even for an unchanged, saturated, or intentionally empty completed audit. Append exactly one result line at the physical bottom of `src/derived/site-content-processing.log` only after synchronization succeeds. On lease or synchronization failure, do not append a completion row; release any acquired lease and report the shared-output blocker without treating the completed shard write as invalid. Always release the lease, and never bypass or remove an active unexpired lease. Do not run generation, repository-wide audits or checks, tests, Astro/Pagefind builds, or any shared command other than topic synchronization.
...
11. Report the audited shard, whether it changed, strengthened content, topic slugs introduced, active creation rules used, unresolved review or ambiguous candidates, the corpus-wide synchronization result, the processing-log line appended, remaining thin ranges, and that the normalization catalog, manual topic-registry editing, corpus-wide topic rewriting, generation, tests, builds, and validation were intentionally not touched. Do not attribute every synchronized record to this shard.
