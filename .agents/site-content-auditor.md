...
10. Report the audited shard, whether it changed, strengthened content, topic slugs introduced, every newly introduced non-decimal adjacent-numeric topic slug that requires repository-owner title and alias review, the processing-log line appended, remaining thin ranges, and that shared topic synchronization, generation, tests, builds, and validation were intentionally not run.
