...
9. After transcript comparison and draft preparation, acquire the repository writer lease immediately before the first canonical shard write and re-read the selected shard to detect drift. Keep the lease across the shard write, `npm run sync:video-topics`, and the processing-log append; pass the token through `CONTENT_PIPELINE_LOCK_TOKEN` so synchronization joins the existing lease. Run synchronization even for an unchanged, saturated, or intentionally empty completed audit. Append exactly one result line at the physical bottom of `src/derived/site-content-processing.log` only after synchronization succeeds. On lease, drift, or synchronization failure, do not append a completion row; release the lease and report the blocker. Always release the lease, and never bypass or remove an active unexpired lease. Do not run generation, repository-wide audits or checks, tests, Astro/Pagefind builds, or any shared command other than topic synchronization.
